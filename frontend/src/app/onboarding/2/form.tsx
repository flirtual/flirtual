...
import { api } from "~/api";
import { PrivacyPreferenceOption, PrivacyPreferenceOptions } from "~/api/user";
...
const privacyOptionLabel: { [K in PrivacyPreferenceOption]: string } = {
...
export const Onboarding2Form: React.FC = () => {
		...
		onSubmit: async (values) => {
			...
			await Promise.all([
				...
				api.profile.update(user.id, {
					...pick(values, ["gender", "sexuality", "games", "languages", "platforms", "interests"]),
	...
	return (
		<form {...formProps} className="flex flex-col gap-8">
			...
			<FormField field={fields.sexualityPrivacy}>
				{(field) => (
					<>
			...
			<FormField field={fields.country}>
				{(field) => (
					<>
			...
			<FormField field={fields.countryPrivacy}>
				{(field) => (
					<>
			...
			<FormField field={fields.languages}>
				{(field) => (
					<>
			...
			<FormField field={fields.platforms}>
				{(field) => (
					<>
			...
			<FormField field={fields.interests}>
				{(field) => (
					<>
