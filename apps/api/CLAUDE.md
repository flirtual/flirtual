...
- **Reports** (`Flirtual.Report`): predefined reasons + optional message/images; Reports tab of the Mod Queue; auto-shadowban after several reports, lifted on clear.
...
- **Moderation events** (`Flirtual.ModerationEvent`): every action and flag above also writes a `moderation_events` row. Lifting one adds its own row *and* stamps `revoked_at`/`revoked_by` on the original, so `active/2` means "still standing". `reviewed_at`/`reviewed_by` are set from the Mod Queue (`/mod`). Reports aren't duplicated here. A flag identical to one since the user's last `warn_acknowledged` is dropped (no event, no Discord) via `ModerationEvent.repeated?/3`.
