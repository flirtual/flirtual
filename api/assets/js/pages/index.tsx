...
type LandingButtonProps = LinkProps & {
			...
			<SnapSection className="bg-brand-gradient px-8 py-16 md:px-16" ref={ref}>
				<div className="mx-auto flex h-full w-full max-w-screen-2xl flex-col items-center justify-center gap-8">
					<div className="flex flex-col items-center justify-center gap-8 text-center">
						<h1 className="font-montserrat mt-8 text-5xl font-bold md:text-7xl">Avatar profiles</h1>
						<span className="font-nunito h-[8ch] max-w-4xl text-2xl leading-snug sm:text-3xl md:h-[5ch] md:text-5xl">
							{activeValue}
						</span>
					</div>
	...
	return (
		<SnapSection className="bg-brand-gradient grid grid-rows-[max-content,1fr,max-content]">
			<div className="flex items-center justify-center p-8 md:p-16">
				<span className="font-montserrat text-3xl md:text-5xl">
...
export const RootIndexPage: React.FC = () => {
	...
	return (
		<div className="h-screen snap-y snap-mandatory overflow-x-hidden scroll-smooth bg-black text-white">
			<SnapSection className="font-montserrat relative flex flex-col items-center justify-center">
				<div className="z-10 flex flex-col items-center px-8 py-16 md:px-16">
					...
					<div className="mt-8 flex flex-col gap-4 gap-y-8 md:flex-row">
						<LandingButton kind="primary" to="/register">
							Sign up
						</LandingButton>
						<LandingButton kind="secondary" to="/login">
							Login
						</LandingButton>
					</div>
				</div>
			...
			<SnapSection className="bg-brand-gradient flex flex-col">
				<div className="flex h-full items-center justify-center p-16 pb-0 md:pb-16">
					<div className="flex flex-col items-center gap-8 md:gap-16">
						<h1 className="font-montserrat text-4xl font-bold sm:text-7xl md:text-8xl">
							Get Flirtual
						</h1>
						<LandingButton kind="secondary-cta" to="/register">
							Sign up
						</LandingButton>
						<div className="grid grid-cols-1 gap-4 gap-y-2 md:grid-cols-2">
							<MobileButton
								Icon={GooglePlayIcon}
								label="Play Store"
								to="https://play.google.com/store/apps/details?id=zone.homie.flirtual.pwa&pcampaignid=pcampaignidMKT-Other-global-all-co-prtnr-py-PartBadge-Mar2515-1"
							/>
							<MobileButton Icon={AppleIcon} label="iPhone/iPad" to="/ios" />
							<MobileButton
								Icon={MicrosoftIcon}
								label="Windows"
								to="https://apps.microsoft.com/store/detail/flirtual/9NWCSDGB6CS3?cid=storebadge&ocid=badge&rtc=1"
							/>
							<MobileButton
								Icon={MetaIcon}
								label="Side Quest"
								to="https://sidequestvr.com/app/9195"
							/>
						</div>
					</div>
				</div>
